...
def reshape_model(model, arch, num_classes):
	...
	elif arch.startswith("mobilenet_v3_large"):
		model.fc = torch.nn.Linear(model.fc.in_features, num_classes)
		print("=> reshaped MobileNetV3_Large fully-connected layer with: " + str(model.fc))

	elif arch.startswith("mobilenet_v3_small"):
		model.fc = torch.nn.Linear(model.fc.in_features, num_classes)
		print("=> reshaped MobileNetV3_small fully-connected layer with: " + str(model.fc))

		
	
	else:
		print("classifier reshaping not supported for this Architecture ")
